...
// A foreign-key descriptor, in the same shape `supabase gen types typescript`
// always produces. @supabase/postgrest-js's internal result-type resolution
// (GetResult) requires this field to be present on every table — without it,
// `.select()` on anything other than "*" silently resolves to `never` instead
// of erroring where you'd notice, which is exactly the bug this fixes.
// This is a type-only field (Database is never instantiated at runtime), so
// no runtime values are needed — just the shape.
  ...
  public: {
    Tables: {
      ...
      trips: {
        ...
        Update: Partial<Trip>;
        Relationships: [DbRelationship, DbRelationship, DbRelationship];
      };
      trip_shares: {
        Row: TripShare;
      ...
      vehicles: {
        Row: Vehicle;
        Insert: Partial<Vehicle> & { owner_id: string; name: string };
        Update: Partial<Vehicle>;
        Relationships: [DbRelationship];
      };
      drivers: {
        Row: Driver;
        Insert: Partial<Driver> & { owner_id: string; full_name: string };
        Update: Partial<Driver>;
        Relationships: [DbRelationship, DbRelationship];
      };
      geofences: {
        Row: Geofence;
      ...
      geofence_events: {
        Row: GeofenceEvent;
        Insert: Omit<GeofenceEvent, "id" | "created_at">;
        Update: never;
        Relationships: [DbRelationship, DbRelationship];
      };
      audit_logs: {
      ...
      push_subscriptions: {
        ...
        Insert: Partial<PushSubscriptionRecord> & { owner_id: string; endpoint: string; p256dh: string; auth_key: string };
      ...
      api_keys: {
        Row: ApiKey;
        Insert: never;
        Update: Partial<Pick<ApiKey, "revoked">>;
        Relationships: [DbRelationship];
      };
      webhooks: {
        Row: Webhook;
